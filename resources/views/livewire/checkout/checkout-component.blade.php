<div class="min-h-screen bg-[#071e3d] p-6">
    <div class="bg-[#e3e9f7] text-black rounded-lg p-4 mb-6">
        <div class="flex justify-between items-start">
            <div>
                <p class="text-lg font-bold text-blue-900">📍 Nama Penerima</p>
                <p>{{ $shippingName ?? 'Penerima belum tersedia' }}</p>
            </div>
            <div>
            ...
            <div>
                <p class="text-lg font-bold text-blue-900">📍 No.Telp Pengiriman</p>
                <p>{{ $shippingNumber ?? 'No.Telp belum tersedia' }}</p>
            </div>
            <div>
                ...
                <span>
                    Rp{{ number_format(
                        $selectedShippingOption 
                            ? ($shippingOptions[$selectedShippingOption]['price'] ?? 0)
                            : 0, 0, ',', '.'
                    ) }}
                </span>
            </div>
            ...
            <div class="flex justify-between font-bold text-lg">
                ...
                @if ($discount > 0)
                   <span class="line-through text-red-500">
                        Rp{{ number_format(
                            $total
                            + ($selectedShippingOption ? ($shippingOptions[$selectedShippingOption]['price'] ?? 0) : 0)
                            + ($serviceFee ?? 2000),
                            0, ',', '.'
                        ) }}
                    </span>
        ...
        <div class="mt-6">
           <button wire:click="placeOrder"
                class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-6 rounded-lg transition duration-200
                    {{ (!$selectedShippingOption || !$paymentMethod) ? 'opacity-50 cursor-not-allowed' : '' }}"
                {{ (!$selectedShippingOption || !$paymentMethod) ? 'disabled' : '' }}>
                Beli Sekarang
            </button>
        </div>
    </div>
