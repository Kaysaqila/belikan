<div class="bg-white min-h-screen">
    ...
    <div class="px-6 mt-6">
<div class="grid grid-cols-6 font-semibold bg-[#002060] text-white rounded-t-lg p-4">
...
@forelse ($cart as $item)
    <div class="grid grid-cols-6 items-center border border-gray-200 p-4 bg-white hover:bg-gray-50 transition-colors" wire:key="row-{{ $item->id }}">
        <!-- Produk -->
        <div class="flex items-center space-x-4">
            <input type="checkbox" wire:model.live="selectedItems" value="{{ $item->id }}" class="checkbox-item w-4 h-4 text-[#002060] rounded focus:ring-[#002060]" />
            <img src="{{ asset('storage/' . $item->product->image) }}" alt="{{ $item->product->name }}" class="w-20 h-20 object-cover rounded-lg shadow-sm" wire:ignore />
            <div>
        ...
        <div class="flex items-center justify-center space-x-3">
            <div class="flex items-center border border-gray-300 rounded-full px-3 py-1">
                <button wire:click="decrementQuantity({{ $item->id }})" class="text-gray-600 hover:text-[#002060] transition-colors w-6 h-6 flex items-center justify-center" title="Kurangi jumlah">
                    <span class="text-lg font-medium">−</span>
                </button>
                <span class="mx-3 text-sm font-medium text-gray-800 min-w-[2rem] text-center">
                    {{ $item->quantity }}
                </span>
                <button wire:click="incrementQuantity({{ $item->id }})" class="text-gray-600 hover:text-[#002060] transition-colors w-6 h-6 flex items-center justify-center" title="Tambah jumlah">
                    <span class="text-lg font-medium">+</span>
                </button>
            </div>
        </div>
        ...
        <div class="flex justify-center">
            <button wire:click="removeItem({{ $item->id }})" class="text-gray-400 hover:text-red-500 transition-colors p-1" title="Hapus item">
...
@empty
    ...
    <div class="text-center py-12 border border-gray-200 rounded-b-lg bg-gray-50">
        ...
        <p class="text-sm text-gray-400 mt-1">Mulai menambahkan produk untuk berbelanja</p>
    </div>
@endforelse
    </div>

    <!-- Voucher dan Total -->
    <div class="bg-gray-200 mt-10 p-4 rounded fixed bottom-0 left-0 w-full z-50 shadow-[0_-2px_8px_rgba(0,0,0,0.08)]">
        <!-- Header Section -->
        <div class="mb-6">
